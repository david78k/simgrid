...
static xbt_dynar_t xbt_dynar_copy(xbt_dynar_t src_dyn)
{
  xbt_dynar_t new_dyn = xbt_dynar_new(src_dyn->elmsize, src_dyn->free_f);

  unsigned long i = 0;
  for (i = 0; i < xbt_dynar_length(src_dyn); i++) {
    void *ptr = xbt_dynar_get_ptr(src_dyn, i);
    xbt_dynar_push(new_dyn, ptr);
  }
...
static void vm_ws_create(const char *name, void *ind_phys_workstation)
{
  ...
  cpu_cas01_create_resource(name, // name
      xbt_dynar_copy(sub_cpu->power_peak_list),        // host->power_peak,
      sub_cpu->pstate,
      1,                          // host->power_scale,
...
{ 
   ...
   /* Update vcpu's action for the new pm */
   {
     ...
     double old_bound = ws_vm2013->cpu_action->bound;
     if (old_bound != 0) {
       XBT_INFO("migrate VM(%s): set bound (%lf) at %s", vm_name, old_bound, pm_name_dst);
       surf_action_set_bound(new_cpu_action, old_bound);
     }
